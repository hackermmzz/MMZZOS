    ...
    for(int i=0;i<blockCnt;++i){
        if(blockOff[i]==0){
            ...
            if(i<12){
                ...
                ide_write(part->my_disk,buff,blockOff[i],1);
                flag=1;
            }
            //如果这正好是第12个块，那么他充当一级间接块
                ...
                *(uint32_t*)buff=data_lba;//一级间接表第一项指向的位置
                ide_write(part->my_disk,buff,parent->inode->i_sectors[i],1);//将位置写入磁盘
                memset(buff,0,SECTOR_SIZE);
                memcpy(buff,entry,sizeof(*entry));
                ide_write(part->my_disk,buff,data_lba,1);//把数据写入磁盘
                flag=1;
            }else{
                //大于12块的先分配数据区,并把数据区指针写入一级间接表
                ide_read(part->my_disk,buff,parent->inode->i_sectors[12],1);//读取一级间接表
                ((uint32_t*)buff)[i-12]=idx0+part->sb->data_lba;
                ide_write(part->my_disk,buff,parent->inode->i_sectors[12],1);//更新一级间接表
                memset(buff,0,SECTOR_SIZE);
                memcpy(buff,entry,sizeof(*entry));
                ide_write(part->my_disk,buff,idx0+part->sb->data_lba,1);//把entry写入扇区
                flag=1;
            }
        }
        else{
            //直接读取对应的扇区
            ide_read(part->my_disk,buff,blockOff[i],1);
            for(int i=0;i<SECTOR_SIZE;i+=sizeof(*entry)){
            ...
            if(flag)ide_write(part->my_disk,buff,blockOff[i],1);//写入磁盘
        }
        if(flag)break;
    }
    if(flag){
